<div>
    ...
    <form wire:submit.prevent="validateForm">
        ...
        <div x-data="{ showSchedule: false }" class="mb-5">
            <!-- Toggle Section -->
                ...
                <div>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" id="toggleSchedule" class="sr-only peer" x-model="showSchedule"
                            x-on:click="$wire.handleSchedule">
                        <div
            ...
            <div x-show="showSchedule" class="my-4 flex flex-col">
                ...
                <label for="start-date" class="mt-2 border border-[#D8D8D894] rounded-2xl p-2">
                    ...
                    <input type="date" id="start-date" name="start-date" placeholder="02/02/2025" wire:model="date"
                        class="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
                    @error('date')
                        <span class="text-red-500 font-bold text-sm"> {{ $message }} </span>
                    @enderror
    ...
    <x-transaction-status :status="$transaction_status" utility="Data" :link="$transaction_link" :modal="$transaction_modal" :action="$isScheduled" />
</div>
@push('scripts')
    <script>
