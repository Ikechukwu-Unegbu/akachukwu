                    ...
                    <form action="{{ route('admin.transfer.bank') }}" method="GET">
                        <div class="mb-4 row">
                            ...
                            <div class="mb-3 col-12 col-md-3">
                                ...
                                <input type="number" name="amountFrom" value="{{ $filters['amountFrom'] ?? '' }}"
                                    placeholder="Min amount" class="form-control">
                            </div>
                            <div class="mb-3 col-12 col-md-3">
                                <label class="form-label">Amount To</label>
                                <input type="number" name="amountTo" value="{{ $filters['amountTo'] ?? '' }}"
                                ...
                                <select name="bank" id="bank" class="form-select">
                                    ...
                                    @foreach ($banks as $bank)
                                        <option value="{{ $bank->code }}"
                                            {{ ($filters['bank'] ?? '') == $bank->code ? 'selected' : '' }}>
                                            {{ $bank->name }}
                                        </option>
                                    @endforeach
                                </select>
            ...
            <div class="card">
                <div class="p-4 card-body">
                    <div class="d-flex justify-content-between">
                        ...
                        <a href="{{ route('admin.transfer.bank') }}?{{ http_build_query(request()->all()) }}&export=pdf"
                    ...
                    <div class="col-md-6 text-end">
                        <form action="{{ route('admin.transfer.bank') }}" method="GET" class="d-inline">
                            ...
                            <select name="perPage" onchange="this.form.submit()"
                                class="w-auto form-select d-inline-block">
                                @foreach ([50, 100, 200] as $perPageOption)
                                    <option value="{{ $perPageOption }}"
                                        {{ ($filters['perPage'] ?? 50) == $perPageOption ? 'selected' : '' }}>
                        ...
                        <tbody>
                            @forelse ($transfers as $transfer)
                                <tr>
                                    <td>
                                        <div class="form-check">
                                            <input class="form-check-input form-check-lg transaction-checkbox"
                                                type="checkbox" value="{{ $transfer->id }}" name="transactions[]">
                                        </div>
                                    </td>
                                    <td>{{ $transfer->reference_id }}</td>
                                    <td>{{ $transfer->sender->username ?? 'N/A' }}</td>
                                    <td> {{ json_decode($transfer->meta ?? '', true)['payeeName'] . '<br />' . $transfer->bank_name ?? 'N/A' }}
                                        <br> {{ $transfer->account_number }}
                                    </td>
