<div id="pinModal" tabindex="-1" aria-hidden="true"
    class="fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full items-center justify-center {{ $formAction ? 'flex' : 'hidden' }}">
        ...
        <div class="relative bg-white text-black rounded-lg shadow dark:bg-gray-700">
            ...
            @if (empty(auth()->user()->pin))
                <div class="p-6 space-y-6 dark:text-white">
                    <h6 class="mb-3 pb-2 dark:text-white">Unable to process transaction. Your PIN is required for this
                        transaction.</h6>
                    <a class="hover:text-red-600 underline mt-3 dark:text-white"
                        href="{{ route('settings.credentials') }}">Click here to create a PIN.</a>
                    <div class="p-6 space-y-6">
                        <h6 class="text-gray-900 mb-3 pb-2">Unable to process transaction. Your PIN is required for this
                            transaction.</h6>
                        <a class="text-gray-900 hover:text-red-600 underline mt-3"
                            href="{{ route('settings.credentials') }}">Click here to create a PIN.</a>
                    </div>
            @endif
                ...
                <form wire:submit="validatePin">
                    <div x-data="pinForm()">
                        ...
                        <div
                            ...
                            <button x-show="isComplete" @click="validatePin" type="submit"
